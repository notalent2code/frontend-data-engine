...
const RevenueModel: FC<RevenueModelProps> = ({ data, baseUrl }) => {
  const axios = useAxiosPrivate();
  const role = useAuthStore((state) => state.session?.role);

  const deleteRevenueModel = async (id: string) => {
    try {
      await axios.delete(`/revenue-model/${id}`);
    ...
    <div className='flex flex-col gap-2'>
      <div className='py-4'>
        <div className='flex flex-col pt-16 lg:pt-0 lg:flex-row items-start lg:items-center justify-between'>
          ...
          {role === 'ADMIN' && (
            <Link
              href={baseUrl + '/revenue-model/create'}
              className={cn(
            ...
            <Card key={item.id} className='flex flex-col gap-2 p-4'>
              <div className='flex flex-row gap-2'>
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger>
                      <Link href={`${baseUrl}/revenue-model/${item.id}/edit`}>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        onClick={() => deleteRevenueModel(item.id)}
                      >
                        Continue
                      </AlertDialogAction>
                    </AlertDialogFooter>
